/**
 * lucide-react removed its brand icons, so the social marks live here
 * as inline SVG. Same `size` prop shape as the lucide icons for consistency.
 */

type IconProps = { size?: number; className?: string };

/**
 * Each mark sits muted alongside its neighbours and returns to its own brand
 * colour when the link is hovered, matching how the Codolio owl behaves.
 *
 * GitHub is the exception: its brand colour is near-black, which would vanish
 * on the dark theme, so it resolves to the foreground colour instead. That is
 * what the mark is designed to do anyway.
 */
const BRAND = {
...
export function Github({ size = 20, className }: IconProps) {
  return (
    <svg
      ...
      fill="currentColor"
      aria-hidden
      className={`transition-colors duration-200 ${BRAND.github} ${className ?? ""}`}
    >
...
export function Leetcode({ size = 20, className }: IconProps) {
  return (
    <svg
      width={size}
      height={size}
      ...
      fill="currentColor"
      aria-hidden
      className={`transition-colors duration-200 ${BRAND.leetcode} ${className ?? ""}`}
    >
...
export function Linkedin({ size = 20, className }: IconProps) {
  return (
    <svg
      ...
      fill="currentColor"
      aria-hidden
      className={`transition-colors duration-200 ${BRAND.linkedin} ${className ?? ""}`}
    >
...
/**
 * Java. simple-icons carries no Java mark for trademark reasons, and the
 * `openjdk` stand-in reads as OpenJDK to anyone who knows the difference and as
 * nothing at all to anyone who doesn't. This is the cup, in Java's colours,
